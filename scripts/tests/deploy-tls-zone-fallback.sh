...
# Asserts: `just release` recipe probes `k3s/<cluster>/cluster.tls.zone`
...
grep -q 'cluster\.tls\.zone' "$JUSTFILE" || {
  echo "FAIL: justfile release recipe lacks cluster.tls.zone probe"
  fail=1
}
grep -q 'global/tls/' "$JUSTFILE" || {
  echo "FAIL: justfile release recipe lacks global/tls/ fallback path"
  fail=1
}
...
for cluster in gxy-management gxy-launchbase; do
  ...
  *)
    echo "FAIL: $marker has invalid zone '$zone'"
    fail=1
    ;;
  esac
done
