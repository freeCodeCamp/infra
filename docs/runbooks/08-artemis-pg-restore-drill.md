...
**Last rehearsed:** 2026-08-25 — PASSED, and now **stale**. The bucket split released on 2026-09-12 and changed the bucket, the token and the `postgres-rclone` image in one go; the rule below requires a re-rehearsal for any one of those. Sections A to F have not run against `backups-gxy-management-cnpg`. **§H has** — it passed against the new bucket, token and image on 2026-09-12, so the pair is covered and only the StatefulSet path is owed a re-run. Triggered by the `postgres-rclone` client-version fix (Helm rev 62, image `@sha256:fbedc38a…`, `pg_dumpall 16.15` matching the live `postgres:16.14-alpine`). Drilled artefact `artemis-20260825-134826.sql.gz`, written by the fixed image. §C `ERRORCOUNT=2` — the two expected `--clean` superuser errors, zero `transaction_timeout`. §D both tenants restored, 6/6 artemis tables, `sites=69` matching the live registry exactly.
...
**Status: PASSED. Run on: 2026-09-12 (twice).** The second run is the one that counts: artefact `artemis-20260912-115655.sql.gz` + its roles file, read from `backups-gxy-management-cnpg` with the backup-only token, written by the `sha256:b9da73ec` image after the bucket split released. Restore `ERROR` lines: **0**. All six tables matched the live pair exactly — `deploys=332 sites=73 aliases=123 outbox=315 tombstones=32 repo_requests=23` against a live read at 11:58:53Z — because the dump was two minutes old. Roles replayed as three guarded `DO` blocks under `ON_ERROR_STOP=1`. The first run, at 07:36Z, read `universe-static-apps-01` with the serve token hours before the split and also passed; its three count deltas each reconciled to a post-dump deletion and are what prompted the §H5 criterion rewrite below.
