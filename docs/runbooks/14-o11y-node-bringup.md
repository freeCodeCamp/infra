...
| 8   | The branch the GitRepository pins is pushed                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | `git ls-remote --heads origin main`                                                                                             |
...
Grafana: <http://ops-vm-o11y-k3s-fra1-01:30300> over Tailscale, or any node's name. The admin password comes from Secret `grafana-admin`, held in 1Password as `mgmt-grafana-admin` and delivered by ExternalSecret. Grafana applies it only when it first creates its database, so an existing install keeps the password it already has and the step-7 database copy carries it across. Set the 1Password item to the password already in use, so a rebuild on an empty volume returns the same login.
