...
| `production: marker not seen` in 2 min            | Caddy `r2_alias` cache (15s) or alias path mismatch; the edge revalidates every request | Check `cache-control` is `max-age=0, must-revalidate` and `cf-cache-status` is `REVALIDATED`/`DYNAMIC` |
