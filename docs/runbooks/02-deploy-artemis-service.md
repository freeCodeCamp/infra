...
sops -d --input-type dotenv --output-type dotenv "$SOT" > "$TMP_DOT"
{
  ...
  while IFS='=' read -r KEY VAL; do
    case "$KEY" in
      R2_ENDPOINT|R2_ACCESS_KEY_ID|R2_SECRET_ACCESS_KEY|R2_BACKUP_ENDPOINT|R2_BACKUP_ACCESS_KEY_ID|R2_BACKUP_SECRET_ACCESS_KEY|GH_CLIENT_ID|JWT_SIGNING_KEY|VALKEY_PASSWORD|SENTRY_DSN|POSTGRES_PASSWORD|ARTEMIS_DB_PASSWORD|HATCHET_DB_PASSWORD)
