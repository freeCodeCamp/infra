# R2 Bucket Provisioning Runbook — `gxy-cassiopeia-1`
...
2. Name: `gxy-cassiopeia-1` (exact match — sequential suffix per D8)
...
2. Token name: `gxy-cassiopeia-1-rw-bootstrap`
3. Permissions: **Object Read & Write**
4. Specify bucket: `gxy-cassiopeia-1`
...
1. Dashboard → R2 → Manage R2 API Tokens → Create API Token
2. Token name: `gxy-cassiopeia-1-ro-caddy`
...
1. Dashboard → `gxy-cassiopeia-1` → Settings → Public access
...
1. Mint a new rw token following step 3 with name `gxy-cassiopeia-1-rw-bootstrap-<YYYYMMDD>`.
...
- [ ] Bucket `gxy-cassiopeia-1` exists in the freeCodeCamp-Universe CF account
- [ ] Versioning: Enabled
...
- [ ] `just r2-bucket-verify gxy-cassiopeia-1` passes all checks
