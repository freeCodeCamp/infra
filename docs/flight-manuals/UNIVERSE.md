...
- Per-galaxy bucket convention (never shared), name `backups-<galaxy>-cnpg`, location hint `weur`: `backups-gxy-management-cnpg` (created 2026-09-12). `backups-gxy-cassiopeia-cnpg` and `backups-gxy-triangulum-cnpg` do not exist yet; create each with hint `weur` when the first CNPG cluster on that galaxy lands.
- `cassiopeia-cnpg-backups` (APAC, empty, no consumer) was deleted 2026-09-12. ADR-008 and ADR-019 were amended 2026-09-12 to the `backups-<galaxy>-cnpg` names.
