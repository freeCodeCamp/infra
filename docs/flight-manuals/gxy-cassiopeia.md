...
Caddy sends two `Cache-Control` policies (`charts/caddy/templates/configmap.yaml`, matchers `@hashed` / `@unhashed`). A hashed asset under `/assets/`, `/_astro/`, `/_next/static/` or `/static/{js,css,media}/` gets `public, max-age=31536000, immutable`; the hash in the filename changes with the content, so a purge is never needed. Every other path — HTML, unhashed scripts, anything at the root — gets `public, max-age=0, s-maxage=86400, must-revalidate`: the browser revalidates on every visit, the Cloudflare edge holds the object for one day, and artemis purges the site hosts 16 s after every alias flip. The zone cache rule `universe-static-apps` (Cache Rules, expression `http.host wildcard "*.freecode.camp" and http.host ne "uploads.freecode.camp"`, edge TTL `bypass_by_default`, browser TTL `respect_origin`; read it back with `GET /zones/<zone>/rulesets/phases/http_request_cache_settings/entrypoint`) reads those headers and bypasses when absent. After an alias write (finalize, promote, rollback, delete, undelete) artemis waits 16 s for Caddy's `r2_alias` cache to expire, then purges the site host at the edge (`POST /zones/<zone>/purge_cache {"hosts":[...]}`, on when `CF_ZONE_ID` and `CF_PURGE_API_TOKEN` are set on the artemis pod; a failed purge logs `edge.purge.failed` and the edge heals on its own when `s-maxage` expires). So stale HTML lasts about 16 s with the purge live, and up to 75 s (60 s edge + 15 s alias cache) without it. Stale content past 75 s means the alias object in R2 still points at the old deploy, not an edge cache. Check the alias, not the CDN:
