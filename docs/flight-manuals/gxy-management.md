...
1. Create `windmill-backup.secrets.env.enc` in infra-secrets (fill
   `PG_PASSWORD` + S3 creds from sample)
2. Deploy:
...
- Secret `windmill-backup-s3` is generated by kustomization from
  `.backup-secrets.env`
