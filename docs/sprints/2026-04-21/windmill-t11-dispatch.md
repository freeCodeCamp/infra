...
2. Encrypts the minted creds into
   `~/DEV/fCC/infra-secrets/constellations/<site>.secrets.env.enc`
   (sops + age) — D34 / Q3 locked path.
3. Registers the same creds as a **repo-scoped** Woodpecker secret
   (NOT org-scoped — D22).
4. Returns deterministic output for `just constellation-register <site>`
...
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **D33 / Q2**       | Admin cred for provisioner lives at `infra-secrets/platform/cf-r2-provisioner.secrets.env.enc`. Flow reads this (via Windmill Resource or env) to obtain CF API token with R2 bucket-admin scope.                                                                                                                        |
| **D34 / Q3**       | Per-site data-plane tokens written to `infra-secrets/constellations/<site>.secrets.env.enc`. Flat path; stable across galaxy moves. `.sops.yaml` creation_rule `path_regex: ^constellations/.*\.secrets\.env\.enc$` — author this rule in the infra-secrets repo as part of T11 delivery (or verify if already present). |
...
- `workspaces/platform/f/static/sops_write.ts` (sops-encrypt wrapper; shell out to `sops` binary inside flow worker)
- `workspaces/platform/f/static/sops_write.test.ts`
...
- Verify (or add): `.sops.yaml` creation_rule for
  `constellations/*.secrets.env.enc` path; age recipient = platform key.
- Seed: `platform/cf-r2-provisioner.secrets.env.enc` with CF API token
  (scoped `R2 Admin Read & Write` on `universe-static-apps-01` only) —
  see "Operator bootstrap" below.
...
## Operator bootstrap (BEFORE T11 starts)

The provisioner admin token does not exist yet. Operator must seed it:

1. In Cloudflare dashboard → R2 → Manage API Tokens → Create Token
   - Name: `r2-provisioner-constellations`
   - Permissions: **Admin Read & Write** on bucket
     `universe-static-apps-01` only (NOT account-wide)
   - Duration: 1 year rotation
2. Save outputs:
   ```
   Access Key ID: <redacted>
   Secret Access Key: <redacted>
   Token value: <redacted>
   Account ID: <operator known>
   ```
3. Encrypt into infra-secrets:
   ```bash
   cd ~/DEV/fCC/infra-secrets
   cat > platform/cf-r2-provisioner.secrets.env <<'DOTENV'
   CF_R2_ADMIN_ACCOUNT_ID=<account-id>
   CF_R2_ADMIN_TOKEN=<token>
   CF_R2_ADMIN_ACCESS_KEY_ID=<akid>
   CF_R2_ADMIN_SECRET_ACCESS_KEY=<sak>
   DOTENV
   sops --encrypt --input-type dotenv --output-type dotenv \
     --output platform/cf-r2-provisioner.secrets.env.enc \
     platform/cf-r2-provisioner.secrets.env
   rm platform/cf-r2-provisioner.secrets.env
   git add platform/cf-r2-provisioner.secrets.env.enc .sops.yaml
   git commit -m "feat(platform): seed cf-r2-provisioner cred (D33)"
   ```
4. Register as Windmill Resource at `u/admin/cf_r2_provisioner` via
   Windmill UI **or** a one-off `wmill resource push` from the flow
   source. Type: custom object shape (see flow code).

Flow reads this Resource each invocation.
