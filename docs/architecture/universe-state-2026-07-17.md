...
| 010 | drifted   | envelope inventory stale (38→32, `k8s/o11y/` deleted 2026-07-14, `r2-read/` undocumented); root `.envrc` loads `global/` repo-wide contra ADR claim; veritas prod envelope keeps GH secrets amendment claimed removed     |
...
| 012 | drifted   | etcd RPO doc 24h vs actual 6h (`0 */6 * * *` in group_vars); Apollo backup row = dead component; registry "recoverable via Git" stale since Postgres cutover (artemis@f115198, 2026-05-10)                                |
...
1. **Secrets** — root `.envrc` loads `global/.env.enc` + `r2-read/.env.enc` on every `cd` (contradicts ADR-010's own security claim; never was true); veritas prod envelope still carries `GITHUB_CLIENT_*`; staging envelope missing.
1. **DR doc rot** — RPO 6h not 24h; Apollo + Git-registry backup rows describe dead components.
