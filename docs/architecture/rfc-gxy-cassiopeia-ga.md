...
| 3     | artemis         | `fCC/artemis`                  | Optional follow-up — (a) extend `POST /api/site/{site}/promote` to accept `{deployId?: string, expectedCurrent?: string}`: when `deployId` set, write directly; when `expectedCurrent` set, refuse on body mismatch (CAS); once landed, deprecate bare-body promote. (b) **NEW per B4 audit-correction:** add `GET /api/site/{site}/alias/{mode}` read-only route + `proxy-client.ts getAlias` method; unblocks the Phase 1 pre-promote alias-body echo that was deferred when discovered unbuildable CLI-only. | B2 service, B4 |
