...
| ----- | --------------- | ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| 1     | universe-cli    | `fCC-U/universe-cli`           | `universe static promote` (no flag) prints a `WARN` envelope: `"You are about to promote deployId=X, which is the current preview alias. To pin a specific deploy, use --from <id>."` (Pre-promote alias-body echo **deferred per B4 audit-correction** — unbuildable CLI-only without a `GET /api/site/{site}/alias/{mode}` route; folded into Phase 3.)                                                                                                                                                                                                                                                                                                                        | B2 default UX  |
...
| 1     | universe-cli    | `fCC-U/universe-cli`           | Post-`deploy --promote` **AND** post-`promote --from <id>` output adds: `"Preview alias unchanged (still <previousDeployId>)."` Surfaces B6 divergence directly to the operator at the moment of the divergent write. **B3 audit-correction:** `siteRollback` (the `--from <id>` route) writes prod alias only, same B6-shape divergence — original RFC §G missed the `--from` surface; warning fires on both.                                                                                                                                                                                                                                                                   | B6, B3         |
| 2     | artemis         | `fCC/artemis`                  | Integration tests: 5 new tests per §G.Verification below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | B4a-e          |
| 3     | artemis         | `fCC/artemis`                  | **Shipped artemis v0.2.0 + universe-cli v0.6.0.** (a) `POST /api/site/{site}/promote` accepts `{deployId?, expectedCurrent?}`; bare-body promote deprecated, warns `promote.legacy_bare` on empty body (artemis `e050648`); CAS landed (`21f403b` `TestPromoteCAS` 200-then-409 alias_drift; `d7457db` `TestPromoteWithDeployId`). (b) `GET /api/site/{site}/alias/{mode}` read-only route + `proxy-client.ts getAlias()` (universe-cli `1450434`); pre-flight body-pin POST (`f476208`), 409 alias_drift surfacing + one-shot retry (`d260fd0`), `AliasDriftError` schema (`0cebc30`), rollback CAS via `expectedCurrent` (`5e3d26d`), bare `siteRollback` retired (`7e7d46d`). | B2 service, B4 |
...
Phase 3 was genuine API surface work in artemis (Go). **Shipped 2026-05-15** as artemis v0.2.0 (chart pin `0.2.0@sha256:038adadab0…`) + universe-cli v0.6.0 (npm published). CAS endpoint + `GET /alias/{mode}` route live; CLI consumes both with pre-flight body-pin + 409 alias_drift retry. Phase 1 telemetry never matured to gate Phase 3 — the audit-correction in §B4 made the alias-read route a hard dependency for Phase 1's body-echo, so Phase 3 shipped first and back-filled Phase 1's deferred capability.

### Audit corrections (2026-05-13)

Deep-source audit during universe-cli Phase 1 implementation surfaced two RFC §G inaccuracies; recorded here so the table above reads consistently with what shipped. Source: `~/DEV/fCC-U/universe-cli/.scratchpad/dossier/closeout/phase-1-cli-promote-divergence.md`.

- **B3** — RFC §G Phase 1 third row originally scoped the `"Preview alias unchanged."` warning to `deploy --promote` only. Empirically, `promote --from <id>` routes through `siteRollback` (`handler/site.go:57-94`) which also writes only the production alias — same B6-shape divergence. Phase 1 commit `75f71e9` fires the warning on both surfaces. RFC table row updated above with `B6, B3` in the Closes column.
- **B4** — RFC §G Phase 1 first row originally promised `"Pre-promote echoes the preview alias body so operator can confirm."` Unbuildable CLI-only: `proxy-client.ts` has no `getAlias`, `server.go:38-50` has no `GET /api/site/{site}/alias/{mode}` route, and `r2.GetAlias` is package-internal to artemis. Folded into Phase 3 row above. Phase 1 ships the WARN envelope without the body echo. Universe-cli operators see only the divergence warning + `--from <id>` hint; the body-echo confirmation lands once the alias-read route exists.

Phase 1 shipped commits (universe-cli `main`): `37af29d` (next-hint `--from <id>`), `75f71e9` (preview/prod divergence WARN on both surfaces), `9a5e50e` + `9f16b7f` (test wiring). Awaits `@freecodecamp/universe-cli` v0.5.1 release.
...
1. Phase 1 CLI changes ship in `@freecodecamp/universe-cli`. **Closed 2026-05-13: universe-cli v0.5.1 published (`96a9cd6` release commit); carries `37af29d`, `75f71e9`, `9a5e50e`, `9f16b7f`. Subsequently superseded by v0.6.0 which adds Phase 3 consumption.**
1. Phase 2 tests land in `fCC/artemis` `internal/integration/` and pass on `make integration`. **Closed 2026-05-14: shipped as artemis v0.2.0 (`9760a06` peeled). Key commits — `21f403b` `TestPromoteCAS` (200-then-409 alias_drift), `d7457db` `TestPromoteWithDeployId` + `TestPromoteRace` frame flip, `e050648` `warn promote.legacy_bare`. `make integration` green on tag build.**
1. Phase 3 service work landed OR parked with re-evaluation trigger. **Closed 2026-05-15: shipped — artemis v0.2.0 carries CAS + `GET /alias/{mode}` route; universe-cli v0.6.0 (`9517d65`) consumes both via `getAlias()` (`1450434`), pre-flight body-pin (`f476208`), 409 alias_drift retry (`d260fd0`), `AliasDriftError` schema (`0cebc30`), rollback CAS via `expectedCurrent` (`5e3d26d`). Telemetry-gate skipped — the §B4 audit-correction made the alias-read route a hard dependency for Phase 1, so service work shipped ahead of CLI feedback rather than after.**
