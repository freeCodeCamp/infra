...
| `smartShutdownTimeout`           | `15`       | Sets the promotion latency directly. Measured 183s against the 180s default, then 18s at 15 — the smart phase always runs its **full** configured length, because an idle client connection is only terminated when the timeout expires. Proven 2026-09-12 from the old primary's log: `FATAL 57P01 terminating connection due to administrator command` at +15s, then `shutdown immediate`, then promotion ~3s later. Every artemis write is one short statement. The field does not roll the instances. |
