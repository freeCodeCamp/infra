-- Create logs_nginx_stg database (staging)
CREATE DATABASE IF NOT EXISTS logs_nginx_stg ON CLUSTER '{cluster}';

-- NGINX access logs table (JSON format from json_analytics)
CREATE TABLE IF NOT EXISTS logs_nginx_stg.access ON CLUSTER '{cluster}'
(
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/logs_nginx_stg/access', '{replica}')
