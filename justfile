...
mirror-artemis-secrets:
    ...
    {
      echo "# Auto-generated by 'just mirror-artemis-secrets' from"
      echo "# ${SOT}. Do NOT hand-edit — re-run the recipe."
      echo
      echo "secretEnv:"
    ...
    echo "Sealed $TGT"
    echo
    echo "Next: commit + push from infra-secrets:"
    echo "  git -C $(dirname $TGT) add $(basename $TGT)"
    echo "  git -C $(dirname $TGT) commit -m 'feat(gxy-management): seal artemis values overlay'"
...
# Deploy/upgrade artemis (Universe deploy proxy on gxy-management). Wraps
# helm install with `--set-file sites=...` injection so the operator never
# types the long path. Sites SOT lives in `freeCodeCamp/artemis` repo at
# `config/sites.yaml`; override default checkout via $ARTEMIS_REPO.
#
# Preconditions:
#   - artemis repo cloned at $ARTEMIS_REPO (default $HOME/DEV/fCC-U/artemis)
#     on the branch + commit that mirrors the deployed image tag.
#   - sealed values overlay landed via `just mirror-artemis-secrets`.
[group('k3s')]
artemis-deploy:
    #!/usr/bin/env bash
    set -eu
    SITES_PATH="${ARTEMIS_REPO:-$HOME/DEV/fCC-U/artemis}/config/sites.yaml"
    [ -f "$SITES_PATH" ] || { echo "Error: $SITES_PATH not found. Pull freeCodeCamp/artemis or set ARTEMIS_REPO."; exit 1; }
    cd k3s/gxy-management
    export KUBECONFIG="$(pwd)/.kubeconfig.yaml"
    CHART_DIR="apps/artemis/charts/artemis"
    HELM_ARGS="-f $CHART_DIR/values.yaml -f apps/artemis/values.production.yaml"
    SECRET_VALUES="{{ secrets_dir }}/k3s/gxy-management/artemis.values.yaml.enc"
    [ -f "$SECRET_VALUES" ] || { echo "Error: missing $SECRET_VALUES — run 'just mirror-artemis-secrets' first"; exit 1; }
    TMPVALS=$(mktemp)
    trap "rm -f $TMPVALS" EXIT
    sops -d --input-type yaml --output-type yaml "$SECRET_VALUES" > "$TMPVALS"
    HELM_ARGS="$HELM_ARGS -f $TMPVALS"
    echo "Deploying artemis (sites: $SITES_PATH)"
    helm upgrade --install artemis "$CHART_DIR" \
      -n artemis --create-namespace \
      $HELM_ARGS \
      --set-file sites="$SITES_PATH"
