  ...
  stackscript_data = {
    userdata = base64encode(
      templatefile("${path.root}/cloud-init--userdata.yml.tftpl", {
        tf_hostname = "api-${count.index + 1}.oldeworld.stg.${local.zone}"
      })
    )
  }
}
...
resource "akamai_dns_record" "stg_oldeworld_api_dnsrecord__vlan" {
  count = local.api_node_count
  ...
  name   = "api-${count.index + 1}.oldeworld.stg.${local.zone}"
  target = [trimsuffix(linode_instance_config.stg_oldeworld_api_config[count.index].interface[1].ipam_address, "/24")]

}

resource "akamai_dns_record" "stg_oldeworld_api_dnsrecord__public" {
  count = local.api_node_count
  ...
  name   = "pub.api-${count.index + 1}.oldeworld.stg.${var.network_subdomain}.${local.zone}"
  target = [linode_instance.stg_oldeworld_api[count.index].ip_address]
}

resource "akamai_dns_record" "stg_oldeworld_api_dnsrecord__private" {
  count = local.api_node_count
  ...
  name   = "prv.api-${count.index + 1}.oldeworld.stg.${local.zone}"
  target = [linode_instance.stg_oldeworld_api[count.index].private_ip_address]
}

resource "cloudflare_record" "stg_oldeworld_api_dnsrecord__vlan" {
  count = local.api_node_count
