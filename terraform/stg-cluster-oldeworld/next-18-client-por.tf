...
resource "linode_instance" "stg_oldeworld_cltpor" {
  ...
  tags = ["stg", "oldeworld", "oldeworld_cltpor"] # Value should use '_' as sepratator for compatibility with Ansible Dynamic Inventory
}
...
resource "linode_instance_config" "stg_oldeworld_cltpor_config" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      # Set the hostname.
      "hostnamectl set-hostname cltpor-${count.index + 1}.oldeworld.stg.${data.linode_domain.ops_dns_domain.domain}",
      "echo \"cltpor-${count.index + 1}.oldeworld.stg.${data.linode_domain.ops_dns_domain.domain}\" > /etc/hostname",
    ]
  }
...
resource "linode_domain_record" "stg_oldeworld_cltpor_config_records__vlan" {
  count = local.cltpor_node_count
...
resource "linode_domain_record" "stg_oldeworld_cltpor_config_records__public" {
  count = local.cltpor_node_count

  domain_id   = data.linode_domain.ops_dns_domain.id
  name        = "pub.cltpor-${count.index + 1}.oldeworld.stg"
  record_type = "A"
  target      = linode_instance.stg_oldeworld_cltpor[count.index].ip_address
  ttl_sec     = 120
}

resource "linode_domain_record" "stg_oldeworld_cltpor_config_records__private" {
  count = local.cltpor_node_count
