...
resource "linode_domain_record" "stg_oldeworld_api_config_records__vlan" {
  count = local.api_node_count
...
resource "linode_domain_record" "stg_oldeworld_api_config_records__public" {
  count = local.api_node_count

  domain_id   = data.linode_domain.ops_dns_domain.id
  name        = "pub.api-${count.index + 1}.oldeworld.stg"
  record_type = "A"
  target      = linode_instance.stg_oldeworld_api[count.index].ip_address
  ttl_sec     = 120
}

resource "linode_domain_record" "stg_oldeworld_api_config_records__private" {
  count = local.api_node_count
