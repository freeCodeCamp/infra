resource "linode_instance" "stg_oldeworld_cltesp" {
  ...
  tags = ["stg", "oldeworld", "stg_oldeworld_clt", "clt", "esp"] # Value should use '_' as sepratator for compatibility with Ansible Dynamic Inventory
}
  ...
  stackscript_data = {
    userdata = base64encode(
      templatefile("${path.root}/cloud-init--userdata.yml.tftpl", {
        tf_hostname = "cltesp-${count.index + 1}.oldeworld.stg.${data.linode_domain.ops_dns_domain.domain}"
      })
    )
  }
}
...
resource "linode_domain_record" "stg_oldeworld_cltesp_dnsrecord__vlan" {
  count = local.cltesp_node_count
...
resource "linode_domain_record" "stg_oldeworld_cltesp_dnsrecord__public" {
  count = local.cltesp_node_count
...
resource "linode_domain_record" "stg_oldeworld_cltesp_dnsrecord__private" {
  count = local.cltesp_node_count
