...
resource "linode_instance" "stg_oldeworld_pxy" {
  ...
  tags = ["stg", "oldeworld", "oldeworld_pxy"] # Value should use '_' as sepratator for compatibility with Ansible Dynamic Inventory
}
...
resource "linode_domain_record" "stg_oldeworld_pxy_config_records__vlan" {
  count = local.pxy_node_count
...
resource "linode_domain_record" "stg_oldeworld_pxy_config_records__public" {
  count = local.pxy_node_count
...
resource "linode_domain_record" "stg_oldeworld_pxy_config_records__private" {
  count = local.pxy_node_count
