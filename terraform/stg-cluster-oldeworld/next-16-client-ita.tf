...
resource "linode_instance_config" "stg_oldeworld_cltita_config" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      # Set the hostname.
      "hostnamectl set-hostname cltita-${count.index + 1}.oldeworld.stg.${data.linode_domain.ops_dns_domain.domain}",
      "echo \"cltita-${count.index + 1}.oldeworld.stg.${data.linode_domain.ops_dns_domain.domain}\" > /etc/hostname",
    ]
  }
...
resource "linode_domain_record" "stg_oldeworld_cltita_config_records__vlan" {
  count = local.cltita_node_count
...
resource "linode_domain_record" "stg_oldeworld_cltita_config_records__public" {
  count = local.cltita_node_count
...
resource "linode_domain_record" "stg_oldeworld_cltita_config_records__private" {
  count = local.cltita_node_count
