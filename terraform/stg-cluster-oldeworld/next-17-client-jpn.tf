resource "linode_instance" "stg_oldeworld_cltjpn" {
  ...
  tags = ["stg", "oldeworld", "stg_oldeworld_clt", "clt", "jpn"] # Value should use '_' as sepratator for compatibility with Ansible Dynamic Inventory
}
  ...
  stackscript_data = {
    userdata = base64encode(
      templatefile("${path.root}/cloud-init--userdata.yml.tftpl", {
        tf_hostname = "cltjpn-${count.index + 1}.oldeworld.stg.${data.linode_domain.ops_dns_domain.domain}"
      })
    )
  }
}
