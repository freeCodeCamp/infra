resource "aws_instance" "stg_mw_nomad_svr" {
  ...
  user_data_base64 = base64encode(templatefile("${path.module}/templates/cloud-init--userdata.yml.tftpl", {
    tf_hostname = "nomad-svr-${count.index + 1}.mw.stg.${local.zone}"
  }))
  ...
  tags = merge(
    var.stack_tags,
    {
      Name = "stg-mw-nomad-svr-${count.index + 1}"
      Role = "nomad_svr"
    }
  )
}

resource "cloudflare_record" "stg_mw_nomad_svr_dnsrecord__private" {
  count = length(aws_instance.stg_mw_nomad_svr)
  ...
  name  = "nomad-svr-${count.index + 1}.mw.stg"
  value = aws_instance.stg_mw_nomad_svr[count.index].private_ip

  depends_on = [aws_instance.stg_mw_nomad_svr]
}
