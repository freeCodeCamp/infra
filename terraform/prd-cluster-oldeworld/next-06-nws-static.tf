  ...
  stackscript_data = {
    userdata = base64encode(
      templatefile("${path.root}/cloud-init--userdata.yml.tftpl", {
        tf_hostname = "jms-${count.index + 1}.oldeworld.prd.${data.linode_domain.ops_dns_domain.domain}"
      })
    )
  }
}
...
resource "linode_domain_record" "prd_oldeworld_jms_dnsrecord__vlan" {
  count = local.jms_node_count
...
resource "linode_domain_record" "prd_oldeworld_jms_dnsrecord__public" {
  count = local.jms_node_count
...
resource "linode_domain_record" "prd_oldeworld_jms_dnsrecord__private" {
  count = local.jms_node_count
