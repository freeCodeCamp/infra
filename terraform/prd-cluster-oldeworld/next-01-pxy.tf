resource "linode_instance" "prd_oldeworld_pxy" {
  ...
  tags = ["prd", "oldeworld", "pxy", "prd_oldeworld_pxy"]
...
resource "linode_instance_disk" "prd_oldeworld_pxy_disk__boot" {
  ...
  image     = local.linode_ubuntu_image
  ...
  stackscript_data = {
    userdata = base64encode(
      templatefile("${path.root}/cloud-init--userdata.yml.tftpl", {
        tf_hostname = "pxy-${count.index + 1}.oldeworld.prd.${local.zone}"
      })
    )
  }
}
...
resource "cloudflare_record" "prd_oldeworld_pxy_dnsrecord__vlan" {
  count = local.pxy_node_count
