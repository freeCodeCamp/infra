...
resource "linode_nodebalancer" "prd_oldeworld_nb_pxy_3" {
  region = var.region
  label  = "prd-nb-oldeworld-pxy-3"
  tags   = ["prd", "oldeworld", "nb_pxy"] # Value should use '_' as sepratator for compatibility with Ansible Dynamic Inventory
}

resource "linode_nodebalancer_config" "prd_oldeworld_nb_pxy_3_config__port_80" {
  nodebalancer_id = linode_nodebalancer.prd_oldeworld_nb_pxy_3.id
  port            = 80
  protocol        = "tcp"
  algorithm       = "leastconn"
  check           = "connection"
...
resource "linode_nodebalancer_config" "prd_oldeworld_nb_pxy_3_config__port_443" {
  nodebalancer_id = linode_nodebalancer.prd_oldeworld_nb_pxy_3.id
  port            = 443
  protocol        = "tcp"
  algorithm       = "leastconn"
